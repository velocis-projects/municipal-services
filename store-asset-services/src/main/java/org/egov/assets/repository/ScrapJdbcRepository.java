...
public class ScrapJdbcRepository extends JdbcRepository {
	...
	public Pagination<Scrap> search(ScrapSearch scrapSearch) {
		String searchQuery = "select * from scrap" + " :condition :orderby";
		...
		if (scrapSearch.getIssueNumber() != null) {
			if (params.length() > 0)
				params.append(" and ");
			params.append("issuenumber in (:issuenumber)");
		...
		List<ScrapEntity> scrapEntities = namedParameterJdbcTemplate.query(searchQuery.toString(), paramValues, row);
