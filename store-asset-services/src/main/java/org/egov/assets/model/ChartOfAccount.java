...
public class ChartOfAccount {
	...
	/**
	 * name is the name of the account code . for example 110 glcode has the name
	 * \"Tax Revenue\"
	 *
	 * @return name
	 **/
	...
	/**
	 * accountCodePurpose is the mapped AccountCodePurpose . This mapping can happen
	 * at any level say Major,minor or detailed. When Account code is searched 1. If
	 * mapped at major code then it lists major and all other codes under that major
	 * code 2. If mapped at minor code then it list minor code and all other codes
	 * under that minor code 3. If mapped at detailed code then it lists only that
	 * code
	 *
	...
	/**
	 * isActiveForPosting true will be considered for transactions. All major,minor
	 * codes will be false and only detailed code will be true . Further any account
	 * code can be disabled for transaction by making this field false
	 *
	...
	/**
	 * parentId is the id of other account code in the chart of account .Chart of
	 * account is created in tree structure. Any code can have other code as parent
	 * . All minor code will have manjor code as parent . All detailed code will
	 * have minor code as parent only leaf account code that is which is not parent
	 * for any account code will used for transactions.
	 *
	 * @return parentId
	 **/
	...
	/**
	 * type is a single character representation of account code type I: Income E:
	 * Expenditure L: Liability A: Asset Account code for all I start with 1 Account
	 * code for all E start with 2 Account code for all L start with 3 Account code
	 * for all A start with 4
	 *
	 * @return type
	 **/
	 ...
	 * Major ,Minor,Subminor or detailed. Major code classification value is 1 Minor
	 * code is 2 Subminor code is 3 Detailed code is 4. Only classification 4 and is
	 * activeforposting=true will be used in transactions. Reports can be generated
	 * at any level.
	 *
	...
	/**
	 * functionRequired field specifies while transacting with this accountcode is
	 * the function is mandatory or not . For any account code this field is set to
	 * true then all transactions expect a fun=ction code to be passed along with
	 * account code
	 *
	...
	/**
	 * budgetCheckRequired field specifies whether budgeting check required for this
	 * account code. Apart from global Budgetcheck configuration this is where
	 * glcode wise budget check decision is made.
	 *
	...
	public int hashCode() {
		return Objects.hash(id, glcode, name, accountCodePurpose, description, isActiveForPosting, parentId, type,
				tenantId, classification, functionRequired, budgetCheckRequired, majorCode, isSubLedger, auditDetails);
	}
