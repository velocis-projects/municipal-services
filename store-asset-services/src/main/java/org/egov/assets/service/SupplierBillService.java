...
import org.egov.assets.model.BillDetail;
...
import org.egov.assets.model.CoaAmountGroup;
...
import org.egov.assets.model.SupplierBillAdvanceAdjustmentSearch;
import org.egov.assets.model.SupplierBillReceipt;
import org.egov.assets.model.SupplierBillReceiptSearch;
...
import org.egov.assets.repository.SupplierBillAdvanceAdjusmentJdbcRepository;
import org.egov.assets.repository.SupplierBillJdbcRepository;
import org.egov.assets.repository.SupplierBillReceiptJdbcRepository;
...
public class SupplierBillService extends DomainService {
	...
	public SupplierBillResponse create(SupplierBillRequest supplierBillRequest, String tenantId) {
		...
		HashMap<String, Material> materialFromMdms = getMaterialFromMdms(tenantId,
				supplierBillRequest.getRequestInfo());
		...
		for (SupplierBill supplierBill : supplierBillList) {
			...
			for (SupplierBillReceipt supplierBillReceipt : supplierBill.getSupplierBillReceipts()) {
				...
				List<MaterialReceiptDetail> receiptDetails = supplierBillReceipt.getMaterialReceipt()
						.getReceiptDetails();
				...
				for (MaterialReceiptDetail materialReceiptDetail : receiptDetails) {
					...
					coaAmountGroups = buildGroupCoa(materialReceiptDetail.getAcceptedQty(),
							materialReceiptDetail.getUnitRate(), glCode);
				}
			}

			for (SupplierBillAdvanceAdjustment supplierBillAdvanceAdjustment : supplierBill
					.getSupplierBillAdvanceAdjustments()) {
				// Set Supplier Bill Advance Adjustment Id
				supplierBillAdvanceAdjustment
						.setId(supplierBillJdbcRepository.getSequence("seq_supplierbilladvanceadjustment"));
				supplierBillAdvanceAdjustment.supplierBill(supplierBill.getId());

				List<SupplierAdvanceRequisition> supplierAdvanceRequisitions = supplierAdvanceRequisitionService
						.create(buildSupplierAdvanceRequisition(supplierBillAdvanceAdjustment))
						.getSupplierAdvanceRequisitions();
			...
			Map<String, BigDecimal> amount = coaAmountGroups.stream()
					.collect(Collectors.groupingBy(coa -> coa.getChartofAccount(),
							Collectors.reducing(BigDecimal.ZERO, CoaAmountGroup::getAmount, BigDecimal::add)));
		...
		return supplierBillResponse.responseInfo(null).supplierBills(supplierBillRequest.getSupplierBills());
	}
	...
	private BillRegister buildBillRegister(String tenantId, List<BillDetail> billDetails,
			Map<String, BigDecimal> coaAmount) {
		...
		while (iterator.hasNext()) {
			...
			BillDetail billDetail = BillDetail.builder().chartOfAccount(new ChartOfAccount().glcode(glCode))
		...
		// Net Payable
		BillDetail detail = BillDetail.builder().chartOfAccount(new ChartOfAccount().glcode(NET_PAYABLE_GLCODE))
				.debitAmount(BigDecimal.ZERO).creditAmount(debitAmount.subtract(creditAmount)).build();

		billDetails.add(detail);

		double billAmountInDouble = billAmount.doubleValue();

		return BillRegister.builder().billDetails(billDetails).tenantId(tenantId).billDate(getCurrentDate())
				.billAmount(billAmountInDouble).billType(SUPPLIER_BILL).passedAmount(billAmountInDouble).build();
	}
	...
	public SupplierBillResponse update(SupplierBillRequest supplierBillRequest, String tenantId) {
		...
		for (SupplierBill supplierBill : supplierBillList) {
			...
			for (SupplierBillAdvanceAdjustment supplierBillAdvanceAdjustment : supplierBill
				...
				List<SupplierAdvanceRequisition> supplierAdvanceRequisitions = supplierAdvanceRequisitionService
						.update(buildSupplierAdvanceRequisition(supplierBillAdvanceAdjustment))
						.getSupplierAdvanceRequisitions();
		...
		if (supplierBills.getPagedData().size() > 0) {
			for (SupplierBill supplierBill : supplierBills.getPagedData()) {
				...
				SupplierBillReceiptSearch supplierBillReceiptSearch = SupplierBillReceiptSearch.builder()
						.supplierBill(supplierBill.getId()).build();

				Pagination<SupplierBillReceipt> billReceiptPagination = supplierBillReceiptJdbcRepository
						.search(supplierBillReceiptSearch);
				...
				SupplierBillAdvanceAdjustmentSearch supplierBillAdvanceAdjustmentSearch = SupplierBillAdvanceAdjustmentSearch
						.builder().supplierBill(supplierBill.getId()).build();

				Pagination<SupplierBillAdvanceAdjustment> advanceAdjustmentPagination = supplierBillAdvanceAdjusmentJdbcRepository
						.search(supplierBillAdvanceAdjustmentSearch);
			...
			return supplierBillResponse.responseInfo(null).supplierBills(supplierBills.getPagedData());

		} else
			return supplierBillResponse.responseInfo(null).supplierBills(Collections.EMPTY_LIST);
	}
	...
	private void validateSameStoreSB(List<SupplierBill> supplierBills, InvalidDataException errors, int i) {
		for (SupplierBill supplierBill : supplierBills) {
			...
			long storeDistinctCount = supplierBill.getSupplierBillReceipts().stream()
					.map(supplierBillReceipt -> supplierBillReceipt.getMaterialReceipt().getReceivingStore().getCode())
					.distinct().count();
		...
		for (SupplierBill supplierBill : supplierBills) {
			...
			for (SupplierBillReceipt supplierBillReceipt : supplierBill.getSupplierBillReceipts()) {
				...
				MaterialReceiptSearch materialReceiptSearch = MaterialReceiptSearch.builder()
						...
						.tenantId(tenantId)
						.receiptType(
								Collections.singletonList(MaterialReceipt.ReceiptTypeEnum.PURCHASE_RECEIPT.toString()))
					...
					for (MaterialReceipt materialReceipt : receiptPagination.getPagedData()) {
						...
						for (MaterialReceiptDetail materialReceiptDetail : materialReceipt.getReceiptDetails()) {
							...
							if (orderDetailEntity != null) {
								...
								throw new CustomException("Purchase Order ", String.format("%s is not found",
										materialReceiptDetail.getPurchaseOrderDetail().getPurchaseOrderNumber()));
							}
						}
					}
				} else {
					throw new CustomException("Material Receipt", "Material receipt "
							+ supplierBillReceipt.getMaterialReceipt().getMrnNumber() + " is not found");
				}
			}
	...
	private SupplierAdvanceRequisitionRequest buildSupplierAdvanceRequisition(
			SupplierBillAdvanceAdjustment supplierBillAdvanceAdjustment) {
		...
		supplierAdvanceRequisitionRequest.setSupplierAdvanceRequisitions(
				Collections.singletonList(supplierBillAdvanceAdjustment.getSupplierAdvanceRequisition()));
	...
	private void backUpdatePo(String tenantId, MaterialReceipt materialReceipt) {
		for (MaterialReceiptDetail materialReceiptDetail : materialReceipt.getReceiptDetails()) {
			...
			purchaseOrderSearch
					.setPurchaseOrderNumber(materialReceiptDetail.getPurchaseOrderDetail().getPurchaseOrderNumber());
			...
			if (purchaseOrderService.checkAllItemsSuppliedForPo(purchaseOrderSearch))
				supplierBillJdbcRepository.updateColumn(new PurchaseOrderEntity(), "purchaseorder", new HashMap<>(),
						"status = 'Fulfilled and Paid'" + " where purchaseordernumber = '"
								+ materialReceiptDetail.getPurchaseOrderDetail().getPurchaseOrderNumber()
	...
	private void backUpdatePoCancelled(String tenantId, MaterialReceipt materialReceipt) {
		for (MaterialReceiptDetail materialReceiptDetail : materialReceipt.getReceiptDetails()) {
			supplierBillJdbcRepository.updateColumn(new PurchaseOrderEntity(), "purchaseorder", new HashMap<>(),
					"status = 'Rejected'" + " where purchaseordernumber = '"
							+ materialReceiptDetail.getPurchaseOrderDetail().getPurchaseOrderNumber()
							+ "' and tenantid = '" + tenantId + "'");
		}
	}
	...
	private void backUpdateReceipt(SupplierBillReceipt supplierBillReceipt) {
		...
		supplierBillJdbcRepository.updateColumn(
				new MaterialReceiptEntity().toEntity(supplierBillReceipt.getMaterialReceipt()), "materialreceipt",
				hashMap, null);
	}
	...
	private List<CoaAmountGroup> buildGroupCoa(BigDecimal acceptedQty, BigDecimal unitRate, String glCode) {
		...
		CoaAmountGroup coaAmountGroup = CoaAmountGroup.builder().amount(amount).chartofAccount(glCode).build();
		coaAmountGroups.add(coaAmountGroup);

		return coaAmountGroups;
	}

	private String getGlCodeForMaterial(String tenantId, HashMap<String, Material> materialFromMdms,
			SupplierBillReceipt supplierBillReceipt, String materialCode) {
		...
		if (null != material && (null == material.getExpenseAccount()
			...
			MaterialStoreMappingSearch storeMappingSearch = MaterialStoreMappingSearch.builder().material(materialCode)
					.store(supplierBillReceipt.getMaterialReceipt().getReceivingStore().getCode()).tenantId(tenantId)
					.build();
			Pagination<MaterialStoreMapping> storeMappingPagination = materialStoreMappingJdbcRepository
					.search(storeMappingSearch);
			...
			if (storeMappingPagination.getPagedData().size() > 0) {
				...
				Pagination<MaterialTypeStoreMapping> typeStoreMappingPagination = materialTypeStoreJdbcRepository
						.search(typeStoreMappingSearch);

				if (typeStoreMappingPagination.getPagedData().size() > 0) {
					for (MaterialTypeStoreMapping materialTypeStoreMapping : typeStoreMappingPagination
							.getPagedData()) {
