		...
		try {
			for (Role roleobj : role) {
				if ((roleobj.getCode()).equalsIgnoreCase(config.getRoleEmployee())) {
					return smid = jdbcTemplate.query(NULMQueryBuilder.GET_SHG_QUERY,
							new Object[] { "", "", shg.getTenantId(),
									shg.getStatus() == null ? "" : shg.getStatus().toString(),
									shg.getStatus() == null ? "" : shg.getStatus().toString(), shg.getShgUuid(),
									shg.getShgUuid() ,
									shg.getStatus() == null ? SmidShgGroup.StatusEnum.DRAFTED.toString()
											: shg.getStatus().toString(),
											shg.getStatus() == null ? SmidShgGroup.StatusEnum.CREATED.toString()
													: shg.getStatus().toString(),
													shg.getStatus() == null ? SmidShgGroup.StatusEnum.DRAFTED.toString()
															: shg.getStatus().toString(),
															shg.getStatus() == null ? SmidShgGroup.StatusEnum.CREATED.toString()
			...
			return smid = jdbcTemplate.query(NULMQueryBuilder.GET_SHG_QUERY, new Object[] { userId.toString(),
					userId.toString(), shg.getTenantId(), shg.getStatus() == null ? "" : shg.getStatus().toString(),
					shg.getStatus() == null ? "" : shg.getStatus().toString(), shg.getShgUuid(), shg.getShgUuid(),"" ,"","",""},
					shgrowMapper);
		} catch (Exception e) {
			e.printStackTrace();
