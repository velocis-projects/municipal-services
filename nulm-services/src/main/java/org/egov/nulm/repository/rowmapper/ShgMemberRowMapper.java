			...
			while (rs.next()) {
				...
				if (!memberMap.containsKey(id)) {
					...
					SmidShgMemberApplication smidapp = SmidShgMemberApplication.builder().auditDetails(audit)
							...
							.applicationStatus(SmidShgMemberApplication.StatusEnum.fromValue(rs.getString("application_status")))
							...
							.wardNo(rs.getString("ward_no")												
							).build();
					List<SmidShgGroup> groupList = null;
					if (rs.getString("group") != null) {
						groupList = Arrays
								.asList(mapper.readValue(rs.getString("group"), SmidShgGroup[].class));
					}
