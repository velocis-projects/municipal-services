...
public class WorkflowIntegrator {
	...
	public void callRoomWorkFlow(BookingsRequest bookingsRequest) {

		String wfTenantId = bookingsRequest.getBookingsModel().getTenantId();

		JSONArray array = new JSONArray();
		BookingsModel bkModel = bookingsRequest.getBookingsModel();
		RoomsModel roomsModel =  bkModel.getRoomsModel().get(0);   
		/*
		 * uuid = commonRepository.findAssigneeUuid(wfTenantId, bkModel.getBkAction(),
		 * bkModel.getBusinessService(), wfTenantId);
		 */
		...
		Map<String, String> uuidmap = new HashMap<>();
		uuidmap.put(UUIDKEY, bkModel.getAssignee());
		obj.put(BUSINESSIDKEY, roomsModel.getRoomApplicationNumber());
		obj.put(TENANTIDKEY, wfTenantId);
		obj.put(BUSINESSSERVICEKEY, roomsModel.getRoomBusinessService());
		obj.put(MODULENAMEKEY, MODULENAMEVALUE);
		obj.put(ACTIONKEY, roomsModel.getAction());
		if (!BookingsFieldsValidator.isNullOrEmpty(roomsModel.getRemarks()))
			obj.put(COMMENTKEY, roomsModel.getRemarks());
		if (!StringUtils.isEmpty(bkModel.getAssignee()))
			obj.put(ASSIGNEEKEY, uuidmap);
		//obj.put(DOCUMENTSKEY, bkModel.getWfDocuments());
		array.add(obj);
		...
		try {
			...
			try {
				...
				log.error("EG_BOOKING_ROOM_WF_ERROR_KEY_NOT_FOUND",
						" Unable to read the json path in error object : " + pnfe.getMessage());
				throw new CustomException("EG_BOOKING_ROOM_WF_ERROR_KEY_NOT_FOUND",
						" Unable to read the json path in error object : " + pnfe.getMessage());
			}
			throw new CustomException("EG_WF_ERROR", errros.toString());
		} catch (Exception e) {
		...
		/*
		 * on success result from work-flow read the data and set the status back to room
		 * object
		 */
		DocumentContext responseContext = JsonPath.parse(response);
		List<Map<String, Object>> responseArray = responseContext.read(PROCESSINSTANCESJOSNKEY);
