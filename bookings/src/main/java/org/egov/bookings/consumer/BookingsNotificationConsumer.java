...
import org.egov.bookings.service.impl.GrievanceService;
import org.egov.bookings.service.impl.NotificationService;
...
public class BookingsNotificationConsumer {
    ...
    public EventRequest prepareuserEvents(Service serviceReq, ActionInfo actionInfo, RequestInfo requestInfo) {
        ...
        for (String role : bookingsUtils.getReceptorsOfNotification(actionInfo.getStatus(), actionInfo.getAction())) {
            if (role.equals(BookingsConstants.ROLE_EMPLOYEE))
                continue;
            ...
            Event event = Event.builder()
                    ...
                    .description(message)
                    .eventType(BookingsConstants.USREVENTS_EVENT_TYPE)
                    .name(BookingsConstants.USREVENTS_EVENT_NAME)
                    .postedBy(BookingsConstants.USREVENTS_EVENT_POSTEDBY)
    ...
    public String getMessageForSMS(Service serviceReq, ActionInfo actionInfo, RequestInfo requestInfo, String role) {
        ...
        if (null == NotificationService.localizedMessageMap.get(locale + "|" + tenantId)) // static map that saves code-message pair against locale | tenantId.
            notificationService.getLocalisedMessages(requestInfo, tenantId, locale, BookingsConstants.LOCALIZATION_MODULE_NAME);
    ...
    public String getMessage(List<Object> listOfValues, String date, Service serviceReq, ActionInfo actionInfo, RequestInfo requestInfo, Map<String, String> messageMap, String role) {
        ...
        if (StringUtils.isEmpty(actionInfo.getStatus()) && !StringUtils.isEmpty(actionInfo.getComment())) {
            text = messageMap.get(BookingsConstants.LOCALIZATION_CODE_COMMENT);
            text = text.replaceAll(BookingsConstants.SMS_NOTIFICATION_COMMENT_KEY, actionInfo.getComment())
                    .replaceAll(BookingsConstants.SMS_NOTIFICATION_USER_NAME_KEY, requestInfo.getUserInfo().getName());
        } else {
            text = messageMap.get(BookingsConstants.getStatusRoleLocalizationKeyMap().get(actionInfo.getStatus() + "|" + role));
            if (actionInfo.getStatus().equals(WorkFlowConfigs.STATUS_OPENED)) {
                if (null != actionInfo.getAction() && actionInfo.getAction().equals(WorkFlowConfigs.ACTION_REOPEN)) {
                    text = messageMap.get(BookingsConstants.getActionRoleLocalizationKeyMap().get(WorkFlowConfigs.ACTION_REOPEN + "|" + role));
                    employeeDetails = notificationService.getEmployeeDetails(serviceReq.getTenantId(), actionInfo.getAssignee(), requestInfo);
                    text = text.replaceAll(BookingsConstants.SMS_NOTIFICATION_EMP_NAME_KEY, employeeDetails.get("name"));
                }
                ...
                text = text.replaceAll(BookingsConstants.SMS_NOTIFICATION_EMP_NAME_KEY, employeeDetails.get("name"))
                        .replaceAll(BookingsConstants.SMS_NOTIFICATION_EMP_DESIGNATION_KEY, designation)
                        .replaceAll(BookingsConstants.SMS_NOTIFICATION_EMP_DEPT_KEY, department);
                ...
                if (reasonForRejection.length < 2)
                    return getDefaultMessage(messageMap, actionInfo.getStatus(), actionInfo.getAction(), actionInfo.getComment());
                text = text.replaceAll(BookingsConstants.SMS_NOTIFICATION_REASON_FOR_REOPEN_KEY, reasonForRejection[0])
                        .replaceAll(BookingsConstants.SMS_NOTIFICATION_ADDITIONAL_COMMENT_KEY, reasonForRejection[1]);
            ...
            if (actionInfo.getStatus().equals(WorkFlowConfigs.STATUS_CLOSED)) {
                ...
                text = text.replaceAll(BookingsConstants.SMS_NOTIFICATION_RATING_KEY,
                        StringUtils.isEmpty(response.getServices().get(0).getRating()) ? "5" : response.getServices().get(0).getRating())
                        .replaceAll(BookingsConstants.SMS_NOTIFICATION_EMP_NAME_KEY, employeeDetails.get("name"));
            }
        }
        if (null != text) {
            String ulb = null;
            ...
            return text.replaceAll(BookingsConstants.SMS_NOTIFICATION_COMPLAINT_TYPE_KEY, listOfValues.get(0).toString())
                    .replaceAll(BookingsConstants.SMS_NOTIFICATION_ID_KEY, serviceReq.getServiceRequestId())
                    .replaceAll(BookingsConstants.SMS_NOTIFICATION_DATE_KEY, date)
                    .replaceAll(BookingsConstants.SMS_NOTIFICATION_APP_LINK_KEY, uiAppHost + uiFeedbackUrl + serviceReq.getServiceRequestId())
                    .replaceAll(BookingsConstants.SMS_NOTIFICATION_APP_DOWNLOAD_LINK_KEY, appDownloadLink)
                    .replaceAll(BookingsConstants.SMS_NOTIFICATION_AO_DESIGNATION, BookingsConstants.ROLE_NAME_GRO)
                    .replaceAll(BookingsConstants.SMS_NOTIFICATION_ULB_NAME, ulb)
                    .replaceAll(BookingsConstants.SMS_NOTIFICATION_SLA_NAME, listOfValues.get(1).toString());
    ...
    public String getDefaultMessage(Map<String, String> messageMap, String status, String action, String comment) {
        ...
        if (StringUtils.isEmpty(status) && !StringUtils.isEmpty(comment)) {
            text = messageMap.get(BookingsConstants.LOCALIZATION_CODE_COMMENT_DEFAULT);
        } else {
            text = messageMap.get(BookingsConstants.LOCALIZATION_CODE_DEFAULT);
            text = text.replaceAll(BookingsConstants.SMS_NOTIFICATION_STATUS_KEY, BookingsConstants.getStatusNotifKeyMap().get(status));
            if (status.equals(WorkFlowConfigs.STATUS_OPENED)) {
                if (null != action && action.equals(WorkFlowConfigs.ACTION_REOPEN))
                    text = text.replaceAll(BookingsConstants.getStatusNotifKeyMap().get(status), BookingsConstants.getActionNotifKeyMap().get(WorkFlowConfigs.ACTION_REOPEN));
            } else if (status.equals(WorkFlowConfigs.STATUS_ASSIGNED)) {
                if (null != action && action.equals(WorkFlowConfigs.ACTION_REASSIGN))
                    text = text.replaceAll(BookingsConstants.getStatusNotifKeyMap().get(status), BookingsConstants.getActionNotifKeyMap().get(WorkFlowConfigs.ACTION_REASSIGN));
            }
        }
