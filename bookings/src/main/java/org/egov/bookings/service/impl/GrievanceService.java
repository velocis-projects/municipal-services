...
import org.egov.bookings.contract.ServiceRequest;
import org.egov.bookings.contract.ServiceRequestDetails;
...
import org.egov.bookings.model.Service.StatusEnum;
import org.egov.bookings.model.user.Citizen;
...
import org.egov.bookings.model.user.UserType;
import org.egov.bookings.producer.BookingsProducer;
import org.egov.bookings.repository.impl.FileStoreRepo;
import org.egov.bookings.repository.impl.IdGenRepo;
import org.egov.bookings.repository.impl.ServiceRequestRepository;
import org.egov.bookings.utils.ErrorConstants;
import org.egov.bookings.utils.BookingsConstants;
import org.egov.bookings.utils.BookingsUtils;
import org.egov.bookings.utils.ResponseInfoFactory;
...
public class GrievanceService {
	...
	private void enrichserviceRequestForcreate(ServiceRequest serviceRequest) {
		...
		List<String> servReqIdList = getIdList(requestInfo, tenantId, serviceReqs.size(), BookingsConstants.SERV_REQ_ID_NAME,
				BookingsConstants.SERV_REQ_ID_FORMAT);
		AuditDetails auditDetails = bookingsUtils.getAuditDetails(String.valueOf(requestInfo.getUserInfo().getId()), true);
	...
	private void overRideCitizenAccountId(ServiceRequest serviceRequest) {
		...
		if (codes.contains(BookingsConstants.ROLE_CITIZEN) || codes.contains(BookingsConstants.ROLE_NAME_CITIZEN))
	...
	private String isUserPresent(Citizen citizen, RequestInfo requestInfo, String tenantId) {
		...
		UserSearchRequest searchRequest = UserSearchRequest.builder().userName(citizen.getMobileNumber())
				.tenantId(tenantId).userType(BookingsConstants.ROLE_CITIZEN).requestInfo(requestInfo).build();
	...
	private void validateAndCreateUser(ServiceRequest serviceRequest) {
		...
		if(roles.contains(BookingsConstants.ROLE_NAME_CSR) || roles.contains(BookingsConstants.ROLE_CSR)) {
	...
	private String createUser(Citizen citizen, RequestInfo requestInfo, String tenantId) {
		...
		citizen.setRoles(Arrays.asList(org.egov.bookings.model.user.Role.builder().code(BookingsConstants.ROLE_CITIZEN).build()));
	...
	private void enrichServiceRequestForUpdate(ServiceRequest request) {
		...
		final AuditDetails auditDetails = bookingsUtils.getAuditDetails(String.valueOf(requestInfo.getUserInfo().getId()),false);
		for (int index = 0; index < serviceReqs.size(); index++) {
			...
			String role = bookingsUtils.getPrecedentRole(requestInfo.getUserInfo().getRoles().stream().map(Role::getCode)
					.collect(Collectors.toList()));
	...
	public Object getServiceRequestDetails(RequestInfo requestInfo, ServiceReqSearchCriteria serviceReqSearchCriteria) {
		...
		try {
			...
			if (e.getMessage().equals(ErrorConstants.NO_DATA_MSG))
				return bookingsUtils.getDefaultServiceResponse(requestInfo);
			else
				throw e;
		}
		searcherRequest = bookingsUtils.prepareSearchRequestWithDetails(uri, serviceReqSearchCriteria, requestInfo);
		Object response = serviceRequestRepository.fetchResult(uri, searcherRequest);
		log.debug(BookingsConstants.SEARCHER_RESPONSE_TEXT + response);
		if (null == response)
			return bookingsUtils.getDefaultServiceResponse(requestInfo);
	...
	public Object getServiceRequestDetailsForPlainSearch(RequestInfo requestInfo, ServiceReqSearchCriteria serviceReqSearchCriteria) {
		StringBuilder uri = new StringBuilder();
		SearcherRequest searcherRequest = null;
		searcherRequest = bookingsUtils.preparePlainSearchReq(uri, serviceReqSearchCriteria, requestInfo);
		Object response = serviceRequestRepository.fetchResult(uri, searcherRequest);
		log.debug(BookingsConstants.SEARCHER_RESPONSE_TEXT + response);
		if (null == response)
			return bookingsUtils.getDefaultServiceResponse(requestInfo);
	...
	public void enrichRequest(RequestInfo requestInfo, ServiceReqSearchCriteria serviceReqSearchCriteria) {
		...
		String precedentRole = bookingsUtils.getPrecedentRole(requestInfo.getUserInfo().getRoles().stream().map(Role::getCode)
				.collect(Collectors.toList()));
		if (requestInfo.getUserInfo().getType().equalsIgnoreCase(BookingsConstants.ROLE_CITIZEN)) {
		...
		} else if (requestInfo.getUserInfo().getType().equalsIgnoreCase(BookingsConstants.ROLE_EMPLOYEE)) {
			/**
			 * GRO can search complaints belonging to only his tenant.
			 */
			if(precedentRole.equalsIgnoreCase(BookingsConstants.ROLE_GRO)) {
			 ...
			 */
			else if (precedentRole.equalsIgnoreCase(BookingsConstants.ROLE_DGRO)) { 
				...
				try {
					List<String> serviceCodes = JsonPath.read(response, BookingsConstants.JSONPATH_SERVICE_CODES);
			 ...
			 */
			else if (precedentRole.equalsIgnoreCase(BookingsConstants.ROLE_EMPLOYEE)) {
			 ...
			 */
			else if (precedentRole.equalsIgnoreCase(BookingsConstants.ROLE_CSR)) {
	...
	public List<String> getDepartmentCode(ServiceReqSearchCriteria serviceReqSearchCriteria, RequestInfo requestInfo) {
		StringBuilder uri = new StringBuilder();
		RequestInfoWrapper requestInfoWrapper = bookingsUtils.prepareRequestForEmployeeSearch(uri, requestInfo,
		...
		try {
			...
			log.debug("Employee: " + response);
			departmenCodes = JsonPath.read(response, BookingsConstants.EMPLOYEE_DEPTCODES_JSONPATH);
		} catch (Exception e) {
	...
	public Object fetchServiceDefs(RequestInfo requestInfo, String tenantId, List<String> departments) {
		StringBuilder uri = new StringBuilder();
		MdmsCriteriaReq mdmsCriteriaReq = bookingsUtils.prepareSearchRequestForServiceCodes(uri, tenantId, departments,
				requestInfo);
		Object response = null;
		try {
	...
	public List<String> getServiceRequestIdsOnAssignedTo(RequestInfo requestInfo,
		...
		SearcherRequest searcherRequest = bookingsUtils.prepareSearchRequestForAssignedTo(uri, serviceReqSearchCriteria,
				requestInfo);
		try {
			Object response = serviceRequestRepository.fetchResult(uri, searcherRequest);
			...
			serviceRequestIds = JsonPath.read(response, BookingsConstants.SRID_ASSIGNEDTO_JSONPATH);
		} catch (Exception e) {
	...
	public Object getCount(RequestInfo requestInfo, ServiceReqSearchCriteria serviceReqSearchCriteria) {
		...
		try {
			...
			if (e.getMessage().equals(ErrorConstants.NO_DATA_MSG))
				return bookingsUtils.getDefaultCountResponse(requestInfo);
			else
				throw e;
		}
		searcherRequest = bookingsUtils.prepareCountRequestWithDetails(uri, serviceReqSearchCriteria, requestInfo);
		Object response = serviceRequestRepository.fetchResult(uri, searcherRequest);
		if (null == response)
			return bookingsUtils.getDefaultServiceResponse(requestInfo);
		Double count = JsonPath.read(response, BookingsConstants.PG_JSONPATH_COUNT);
				...
				if(!StringUtils.isEmpty(tenantId)) {
					...
					if(!CollectionUtils.isEmpty(mapOfTenantIdAndMohallaCodes.get(tenantId))) {
						tenantWiseMap = getMohallNames(requestInfo, tenantId, mapOfTenantIdAndMohallaCodes.get(tenantId), 
								BookingsConstants.LOCATION__BOUNDARY_HIERARCHYTYPE_ADMIN, BookingsConstants.LOCATION__BOUNDARY_BOUNDARYTYPE_LOCALITY);
					}
	...
	public UserResponse getUsers(RequestInfo requestInfo, String tenantId, List<Long> userIds) {
		...
		UserSearchRequest searchRequest = UserSearchRequest.builder().id(userIds).tenantId(tenantId)
				.userType(BookingsConstants.ROLE_CITIZEN).requestInfo(requestInfo).build();
	...
	public Map<String, String> getMohallNames(RequestInfo requestInfo, String tenantId, List<String> mohallaCodes, String hierarchyType, String boundaryType){
		StringBuilder uri = new StringBuilder();
		RequestInfoWrapper request = bookingsUtils.prepareRequestForLocation(uri, requestInfo, boundaryType, tenantId, hierarchyType, mohallaCodes);
		Map<String, String> map = new HashMap<>();
		try {
			Object response = serviceRequestRepository.fetchResult(uri, request);
			if(null != response) {
				List<String> names = JsonPath.read(response, BookingsConstants.LOCATION__BOUNDARY_NAMES_JSONPATH);
				List<String> codes = JsonPath.read(response, BookingsConstants.LOCATION__BOUNDARY_CODES_JSONPATH);
