...
public class EcQueryBuilder {
	...
	public static final String GET_VIOLATION_MASTER = "select (select case when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1)< now()- interval '30 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION'  when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1) < now()- interval '365 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type = 'Seizure of Vehicles' then 'PENDING FOR AUCTION' else challan.challan_status end )as challan_status,*,(select head_amount from egec_challan_detail ch where ch.budget_head ='FINE_AMOUNT' and ch.challan_uuid=challan.challan_uuid) as fineAmount, \n"
			...
			+ " from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment\n"
			+ "where violation.violation_uuid = item.violation_uuid and\n"
			+ "violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.violation_uuid = payment.violation_uuid and violation.tenant_id=? order by violation.last_modified_time desc";

	public static final String GET_VIOLATION_MASTER_AUTION = "select (select case when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1)< now()- interval '30 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION'  when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1) < now()- interval '365 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type = 'Seizure of Vehicles' then 'PENDING FOR AUCTION' else challan.challan_status end )as challan_status,\n"
			...
			"(select head_amount from egec_challan_detail ch where ch.budget_head ='PENALTY_AMOUNT' and ch.challan_uuid=challan.challan_uuid) as penaltyAmount\n" + 
			"from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment,public.egec_store_item_register storeItem\n" + 
			"where violation.violation_uuid = item.violation_uuid and storeItem.violation_uuid = violation.violation_uuid and\n" + 
			"violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid  and violation.tenant_id=?\n" + 
			"and violation.violation_uuid = payment.violation_uuid\n" + 
			"and ((violation.encroachment_type <> 'Seizure of Vehicles' and storeItem.item_store_deposit_date < now()- interval '30 days') OR (violation.encroachment_type = 'Seizure of Vehicles' and storeItem.item_store_deposit_date < now()- interval '365 days')) and ((payment.payment_status = 'PENDING' and violation.encroachment_type <> 'Unauthorized/Unregistered Vendor') OR (violation.encroachment_type = 'Unauthorized/Unregistered Vendor')) and challan.challan_status <> 'CLOSED' order by violation.last_modified_time desc";
	...
	public static final String GET_VIOLATION_MASTER_SM = "select (select case when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1)< now()- interval '30 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION'  when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1) < now()- interval '365 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type = 'Seizure of Vehicles' then 'PENDING FOR AUCTION' else challan.challan_status end )as challan_status,\n"
			...
			"			 from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment\n" + 
			"			where violation.violation_uuid = item.violation_uuid and\n" + 
			"			violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.violation_uuid = payment.violation_uuid and violation.tenant_id=? \n" + 
			"			and challan.challan_status not in  ('CHALLAN ISSUED', 'CLOSED') order by violation.last_modified_time desc";
	...
	public static final String GET_VIOLATION_MASTER_SEARCH = "select (select case when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1)< now()- interval '30 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION'  when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1) < now()- interval '365 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type = 'Seizure of Vehicles' then 'PENDING FOR AUCTION' else challan.challan_status end )as challan_status,\n"
			...
			+ "    		where violation.contact_number ilike ? or challan.challan_id ilike ? \n"
	...
	public static final String GET_ITEM_AGING_REPORT4 = "select item_name, (quantity -auctioned_quantity)as item_quantity,challan.challan_id,item_store_deposit_date, (now()::date - item_store_deposit_date::date)::int as age, (select case when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1)< now()- interval '31 days' and challan.challan_status <> 'CLOSED' and master.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION' when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1) < now()- interval '365 days' and challan.challan_status <> 'CLOSED' and master.encroachment_type = 'Seizure of Vehicles' then 'PENDING FOR AUCTION' else challan.challan_status end )as challan_status from egec_store_item_register store, egec_violation_master master, egec_challan_master challan where master.violation_uuid=store.violation_uuid and challan.violation_uuid=store.violation_uuid and item_store_deposit_date < now()- interval '31 days'  and item_store_deposit_date > now()- interval '10000 days'  and challan.challan_status <> 'CLOSED' and master.tenant_id=? order by master.created_time";
	public static final String GET_ITEM_AGING_REPORT5 = "select item_name, (quantity -auctioned_quantity)as item_quantity,challan.challan_id,item_store_deposit_date, (now()::date - item_store_deposit_date::date)::int as age, challan.challan_status from egec_store_item_register store, egec_violation_master master, egec_challan_master challan where master.violation_uuid=store.violation_uuid and challan.violation_uuid=store.violation_uuid and challan.challan_status <> 'CLOSED' and master.tenant_id=? order by master.created_time";
	...
	public static final String GET_VIOLATION_MASTER_HOD = "select (select case when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1)< now()- interval '30 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type <> 'Seizure of Vehicles' then 'PENDING FOR AUCTION'  when (select store.item_store_deposit_date from egec_store_item_register store where store.challan_uuid=challan.challan_uuid limit 1) < now()- interval '365 days' and challan.challan_status <> 'CLOSED' and violation.encroachment_type = 'Seizure of Vehicles' then 'PENDING FOR AUCTION' else challan.challan_status end )as challan_status,\n"
			+ " *,(select head_amount from egec_challan_detail ch where ch.budget_head ='FINE_AMOUNT' and ch.challan_uuid=challan.challan_uuid) as fineAmount, \n" + 
			"			(select head_amount from egec_challan_detail ch where ch.budget_head ='PENALTY_AMOUNT' and ch.challan_uuid=challan.challan_uuid) as penaltyAmount \n" + 
			"			 from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment\n" + 
			"			where violation.violation_uuid = item.violation_uuid and\n" + 
			"			violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.violation_uuid = payment.violation_uuid \n" + 
	...
	public static final String GET_VIOLATION_MASTER_AUCTION_HOD="select *,(select head_amount from egec_challan_detail ch where ch.budget_head ='FINE_AMOUNT' and ch.challan_uuid=challan.challan_uuid) as fineAmount,  \n"
			+ "    		(select head_amount from egec_challan_detail ch where ch.budget_head ='PENALTY_AMOUNT' and ch.challan_uuid=challan.challan_uuid) as penaltyAmount \n"
			+ "    		  from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_auction_master auction, egec_payment payment\n"
			+ "        	where violation.violation_uuid = item.violation_uuid and\n"
			+ "    		violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.tenant_id=?\n"
			+ "			and violation.violation_uuid = auction.violation_uuid and violation.violation_uuid = payment.violation_uuid and auction.status='PENDING' order by violation.last_modified_time desc";
	...
	public static final String GET_AUCTIONED_AVAILABLE_COUNT ="select b.challan_uuid,b.challan_id from (select ( (select count(*) from egec_store_item_register sr where sr.quantity::int != auctioned_quantity::int  and sr.challan_uuid = a.challan_uuid ) +(select count(*) from egec_auction_master am where am.status = 'PENDING' and am.challan_uuid = a.challan_uuid )) as quantityCount ,a.challan_uuid,a.challan_id from (select * from egec_challan_master where challan_status = 'ADDED TO STORE' and tenant_id = ?) a group by a.challan_uuid,a.challan_uuid,a.challan_id) b where b.quantityCount = 0 ";
	...
	public static final String GET_DASHBOARD_DETAILS_SM = "select (select count(distinct challan.challan_id) from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment \n" +
			 " where violation.violation_uuid = item.violation_uuid and violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.violation_uuid = payment.violation_uuid and violation.tenant_id=? and challan.challan_status not in ('CLOSED' ,'CHALLAN ISSUED')) as challanCount,\n" +
			 "(select count(distinct challan.challan_id)\n" + 
			 "			from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment,public.egec_store_item_register storeItem\n" + 
			 "			where violation.violation_uuid = item.violation_uuid and storeItem.violation_uuid = violation.violation_uuid and\n" + 
			 "			violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid  and violation.tenant_id=?\n" + 
			 "			and violation.violation_uuid = payment.violation_uuid\n" + 
			 "			and ((violation.encroachment_type <> 'Seizure of Vehicles' and storeItem.item_store_deposit_date < now()- interval '30 days') OR (violation.encroachment_type = 'Seizure of Vehicles' and storeItem.item_store_deposit_date < now()- interval '365 days')) and ((payment.payment_status = 'PENDING' and violation.encroachment_type <> 'Unauthorized/Unregistered Vendor') OR (violation.encroachment_type = 'Unauthorized/Unregistered Vendor')) and challan.challan_status <> 'CLOSED') as auctionCount";
	...
	public static final String GET_DASHBOARD_DETAILS_SI_SM="select(select count(distinct challan.challan_id) as challanCount from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment \n" +
			 " where violation.violation_uuid = item.violation_uuid and violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.violation_uuid = payment.violation_uuid and violation.tenant_id=?) as challanCount, \n" +
			 "(select count(distinct challan.challan_id)\n" + 
			 "			from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment,public.egec_store_item_register storeItem\n" + 
			 "			where violation.violation_uuid = item.violation_uuid and storeItem.violation_uuid = violation.violation_uuid and\n" + 
			 "			violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid  and violation.tenant_id=?\n" + 
	...
	public static final String GET_DASHBOARD_DETAILS_SI_HOD="select(select count(distinct challan.challan_id) as challanCount from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment \n" +
			 " where violation.violation_uuid = item.violation_uuid and violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.violation_uuid = payment.violation_uuid and violation.tenant_id=?) as challanCount, \n" +
	...
	public static final String GET_DASHBOARD_DETAILS_SM_HOD="select (select count(distinct challan.challan_id) from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment \n" +
			 " where violation.violation_uuid = item.violation_uuid and violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.violation_uuid = payment.violation_uuid and violation.tenant_id=? and challan.challan_status not in ('CLOSED' ,'CHALLAN ISSUED')) as challanCount,\n" +
			 "(select count(*) from egec_fine_master where tenant_id=? and approval_status='PENDING'  and is_active = 'TRUE'::boolean) as fineCount, \n" 
	...
	public static final String GET_DASHBOARD_DETAILS_SI_SM_HOD="select (select count(distinct challan.challan_id) from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_payment payment \n" + 
			" where violation.violation_uuid = item.violation_uuid and violation.violation_uuid = doc.violation_uuid and violation.violation_uuid=challan.violation_uuid and violation.violation_uuid = payment.violation_uuid and violation.tenant_id=?) as challanCount,\n" +
			 "(select count(*) from egec_fine_master where tenant_id=? and approval_status='PENDING'  and is_active = 'TRUE'::boolean) as fineCount, \n" 
			 + " 			(select (select count(distinct auction.auction_uuid)  \n"
				+ "    		from public.egec_violation_master violation, public.egec_violation_detail item, public.egec_document doc, egec_challan_master challan, egec_auction_master auction, egec_payment payment\n"
